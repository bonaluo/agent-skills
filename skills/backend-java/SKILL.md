---
name: backend-java
description: Spring Boot + Maven + MySQL + MyBatis Java backend full-stack skill, including project initialization, structure design, MyBatis practice, Flyway database versioning, containerized deployment, etc. / Spring Boot + Maven + MySQL + MyBatis Java 后端全栈技能，包含项目初始化、结构设计、MyBatis实践、Flyway数据库版本管理、容器化部署等。
metadata:
  version: 20260417.0000
  update-url: https://github.com/bonaluo/agent-skills@backend-java
---

# backend-java

Spring Boot + Maven + MySQL + MyBatis Java 后端开发全栈技能。

## When to use

开发 Java 后端服务时使用，包括：
- 创建新的 Spring Boot 应用
- 开发 RESTful API
- 数据库 CRUD 操作
- 后端微服务开发

## 技术栈

| 组件 | 技术选型 |
|------|----------|
| 框架 | Spring Boot / Spring Cloud |
| 构建 | Maven |
| 数据库 | MySQL |
| ORM | MyBatis / MyBatis-Plus |
| Java | JDK 17+ |

## 相关技能

- [项目初始化](references/project-init.md)
- [项目结构](references/project-structure.md)
- [MyBatis 最佳实践](references/database-config.md)
- [Flyway 数据库版本管理](references/database-versioning.md)
- [容器化最佳实践](references/containerization.md)
